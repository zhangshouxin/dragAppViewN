		...
		watch: {
			...
			triggerSaveModal(v1,v2){
				...
				uni.showModal({
					title: '保存页面',
					content: '',
					placeholderText:"页面名称(英文)@页面标题(中文)",
					editable:true,
						...
						if (res.confirm) {
							...
							if(null!=pageName&&pageName!=""){
								if(pageName.indexOf("@")>0){
									let _list=pageName.split("@");
								    if(_this.$u.test.isEmpty(_list[0])){
										uni.showToast({
											title: '页面名称不能为空',
											duration: 2000,
											icon:"error"
									...
									else if(_this.$u.test.isEmpty(_list[1])){
										uni.showToast({
											title: '页面标题不能为空',
											duration: 2000,
											icon:"error"
										...
										_this.z_modal_title=_list[1];
										_this.z_modal_name=_list[0];
										_this.modalConfirm()
									}
									...
									uni.showToast({
										title: '输入格式不正确，格式为页面名称(英文)@页面标题(中文)',
										duration: 2000,
										icon:"error"
									});
								}
